...
class Apis
{
    ...
    */
    ...
    /**
     * 수집된 데이터 삭제
     * collection의 모든 데이터를 삭제합니다.
     */
    public function deleteCollectedData()
    {
        try {
            ...
            if (empty($collection)) {
                throw new \Exception('컬렉션 이름이 제공되지 않았습니다.');
            }
            ...
            // 모든 수집된 데이터 삭제
            $result = $this->db->delete($collection, $filter);
            
            $data['success'] = true;
            $data['deletedCount'] = $result->getDeletedCount();
            $data['message'] = $result->getDeletedCount() . '개의 데이터가 삭제되었습니다.';
    ...
    public function getUrlData(){
        ...
        try {
            ...
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // HTTPS 인증서 무시 (보안상 실제 운영에서는 주의)

            //curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
            // User-Agent를 명시적으로 설정하여 일부 API 서버에서 차단되지 않도록 함
            ...
            if (!empty($requestHeaders)) {
                // 헤더 라인 내 불필요한 공백 및 콜론 형식 표준화
                $normalizedHeaders = [];
                foreach ($requestHeaders as $headerLine) {
                    // 콜론 앞뒤로 공백이 있는 경우도 있으므로, 콜론 기준 split 후 양쪽 트리밍
                    if (strpos($headerLine, ':') !== false) {
                        ...
                        // 콜론 없는 경우에는 그대로 추가(비정상적인 헤더)
                        $normalizedHeaders[] = trim($headerLine);
                    }
                }
