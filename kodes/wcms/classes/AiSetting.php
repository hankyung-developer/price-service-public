...
class AiSetting
{
    ...
    public function scheduleInsert(){
        try {
            ...
            if(empty($request['templateId'])){
                return [
                    'success' => false,
                    'message' => '기사 템플릿을 선택해주세요.'
                ];
            }
            
            if(empty($request['promptId'])){
                return [
                    'success' => false,
            ...
            if(!empty($request['scheduleConfig'])){
                $request['scheduleConfig'] = json_decode($request['scheduleConfig'], true);
            }
            
            // selectedItems JSON 파싱
            if(!empty($request['selectedItems'])){
                $request['selectedItems'] = json_decode($request['selectedItems'], true);
            }
            ...
            // dataPeriod 정수 변환 및 기본값 설정
            if(!empty($request['dataPeriod'])){
                $request['dataPeriod'] = (int)$request['dataPeriod'];
            } else {
                $request['dataPeriod'] = 7; // 기본값: 1주일
            }
            
            // makeImage, makeChart 기본값 설정
            if(empty($request['makeImage'])){
                $request['makeImage'] = 'generate';
            }
            if(empty($request['makeChart'])){
                $request['makeChart'] = 'generate';
            }
            ...
            return [
                'success' => false,
                'message' => $this->common->getExceptionMessage($e)
            ];
        }
    }
    ...
    public function scheduleDelete(){
        try {
            ...
            $request['delete']['managerId'] = $_SESSION['managerId'];
            $request['delete']['managerName'] = $_SESSION['managerName'];
            $request['delete']['date'] = date('Y-m-d H:i:s');
